...
2. **Define system message**: We'll use the following prompt to guide our HR assistant to generate helpful answers to employee queries.
   ...
   <Note>
   Using Maxim, we can define [dynamic variables](/prompt-engineering/prompt-playground#using-variables) in the prompt. Here, we'll attach our context source as the value for the <code>`{{context}}`</code> variable.
   </Note>
...
4. **Connect the context source** to our prompt in the [Prompt Playground](/prompt-engineering/prompt-playground), under the "Variables" section. Also, select an LLM of choice (using Gemini 2.0 flash in this example).

5. **To test the output**, we'll pass the following user query to our HR assistant:
...
We'll evaluate the performance using the following evaluators from Maxim's [Evaluator Store](/library/evaluators/pre-built-evaluators/overview):
