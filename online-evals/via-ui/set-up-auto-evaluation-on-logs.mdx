...
**Examples:** Conversation coherence, multi-turn faithfulness, overall user satisfaction, goal completion rate
...
- Your evaluator only needs the current input and output

**Examples:** Response helpfulness, answer accuracy, tone appropriateness, hallucination detection
...
**Examples:** Retrieval relevance, generation clarity, tool call appropriateness, context quality
...
This hierarchical view shows the structure of your trace and is essential when you have evaluations at different levels:

- **Session level**: The root node represents the entire session
- **Trace level**: Child nodes represent individual traces within a session
- **Span level**: Nested nodes represent components within traces (generations, retrievals, tool calls, etc.)
