...
Dataset splits allow you to create subsets of a larger dataset for different use cases or logical groupings. Creating a split helps manage datasets more easily and facilitates testing of specific prompts or endpoints. You can create splits within a dataset, add elements to them, and view them side by side under different tabs. Learn more about dataset splits [here](/offline-evals/via-ui/prompts/human-annotation).

## Prompt partials

Prompt partials act as reusable snippets that you can include across different Prompts. They help eliminate repetitive content by maintaining common prompt elements in a single place. Learn more about prompt partials [here](/library/prompt-partials/create-prompt-partial).