...
Choose the model that powers your simulation agent. We recommend GPT-4.1 for its strong instruction-following capabilities and low latency. You can also configure parameters like temperature, max tokens, response format - the same options available in the [prompt playground](/offline-evals/via-ui/prompts/prompt-playground#configuring-parameters).
